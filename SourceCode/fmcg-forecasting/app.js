...
// LOAD FILE INFO (sizes, dates, etc.)
async function loadFileInfo() {
    const files = [
        {
            name: 'xgb_baseline_gpu_metrics.csv',
            path: 'data/processed/xgb_baseline_gpu_metrics.csv'
        },
        {
            name: 'xgb_baseline_gpu_test_predictions.csv',
            path: 'data/processed/xgb_baseline_gpu_test_predictions.csv'
        }
    ];
...
function handleError(error) {
    ...
    // Set fallback values
    const elements = ['model-mae', 'model-rmse', 'model-r2', 'models-mae', 'models-rmse', 'models-r2', 'models-mape'];
    elements.forEach(id => {
...
function openInKaggle() {
    // Replace with your actual Kaggle notebook URL
    window.open('https://www.kaggle.com/code/yournotebook', '_blank');
}
...
function updateSystemStatus() {
    const lastSyncElement = document.querySelector('.metric-row .value');
    if (lastSyncElement) {
        const now = new Date();
        const timeStr = now.toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' });
        console.log(`🔄 System check: ${timeStr}`);
    }
}
